  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (response.status === 404) {
        console.warn(`Resource not found (404), continuing gracefully: ${url}`);
        return response;
      }
      ...
      console.log(`Retryable error ${response.status}. Retrying in ${delay.toFixed(0)}ms...`);
      await new Promise(resolve => setTimeout(resolve, delay));

    } catch (error) {
      // This catches network errors (e.g., DNS failure, no internet)
      console.error(`Network error during fetch attempt ${attempt + 1}:`, error);
      if (attempt === maxRetries - 1) {
