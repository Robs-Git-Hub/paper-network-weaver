...
// REFACTOR: Import the new EnrichedPaper type. `Author` and `useKnowledgeGraphStore` are no longer needed here.
import { Paper } from '@/store/knowledge-graph-store';
import { EnrichedPaper } from './MainAnalysisView';
import { useIsMobile } from '@/hooks/use-mobile';

// REFACTOR: Update props to accept the new EnrichedPaper type.
...
// REFACTOR: Rename component to UnifiedCitationsTable.
export const UnifiedCitationsTable: React.FC<UnifiedCitationsTableProps> = ({ papers }) => {
  // REFACTOR: `getAuthorsForPaper` is no longer needed, so we don't need access to the store here.
  const isMobile = useIsMobile();
  const [sorting, setSorting] = useState<SortingState>([
    { id: 'citations', desc: true },
  ]);

  // REFACTOR: This function is now redundant. The selector in MainAnalysisView provides the authors.
  // const getAuthorsForPaper = ...
  ...
  // REFACTOR: The column definition now operates on the EnrichedPaper type.
  const columns = useMemo<ColumnDef<EnrichedPaper>[]>(() => [
    ...
    },
    {
      ...
      header: 'Authors',
      cell: ({ row }) => {
        // FIX: Read authors directly from the enriched paper object.
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-wrap gap-1">
          {/* FIX: Read relationship_tags directly from the enriched paper object. */}
  ...
  if (isMobile) {
    return (
      <div className="space-y-4">
        ...
        {papers.map((paper) => {
          // FIX: Read authors directly from the enriched paper object for the mobile view.
